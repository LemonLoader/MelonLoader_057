...
{
    public class MonkiiPreferences_Category
    {
        public readonly List<MonkiiPreferences_Entry> Entries = new List<MonkiiPreferences_Entry>();
        ...
        internal MonkiiPreferences_Category(string identifier, string display_name, bool is_hidden = false, bool is_inlined = false)
        {
            Identifier = identifier;
            DisplayName = display_name;
            IsHidden = is_hidden;
            IsInlined = is_inlined;
            MonkiiPreferences.Categories.Add(this);
        }

        public MonkiiPreferences_Entry CreateEntry<T>(string identifier, T default_value, string display_name, bool is_hidden) 
            => CreateEntry(identifier, default_value, display_name, null, is_hidden, false, null, null);
        public MonkiiPreferences_Entry<T> CreateEntry<T>(string identifier, T default_value, string display_name,
        ...
        public MonkiiPreferences_Entry<T> CreateEntry<T>(string identifier, T default_value, string display_name = null,
            string description = null, bool is_hidden = false, bool dont_save_default = false, Preferences.ValueValidator validator = null, string oldIdentifier = null)
        {
            ...
            entry = new MonkiiPreferences_Entry<T>
            {
                Identifier = identifier,
                DisplayName = display_name,
        ...
        {
            MonkiiPreferences_Entry entry = GetEntry(identifier);
            if (entry != null)
        ...
        public MonkiiPreferences_Entry GetEntry(string identifier)
        {
            if (string.IsNullOrEmpty(identifier))
        ...
        public MonkiiPreferences_Entry<T> GetEntry<T>(string identifier) => (MonkiiPreferences_Entry<T>)GetEntry(identifier);
        ...
        public void SaveToFile(bool printmsg = true)
        {
            ...
            if (currentfile == null)
                currentfile = MonkiiPreferences.DefaultFile;
            foreach (MonkiiPreferences_Entry entry in Entries)
