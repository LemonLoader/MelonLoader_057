...
{
    public static class MonkiiUtils
    {
        ...
        internal static void Setup(AppDomain domain)
        {
            ...
            MonkiiLoaderDirectory = Path.GetDirectoryName(typeof(MonkiiUtils).Assembly.Location);

            MonkiiLaunchOptions.Load();
            MonkiiHandler.Setup();
            UnityInformationHandler.Setup();

            CurrentGameAttribute = new MonkiiGameAttribute(UnityInformationHandler.GameDeveloper, UnityInformationHandler.GameName);
#if __ANDROID__
            CurrentPlatform = MonkiiPlatformAttribute.CompatiblePlatforms.ANDROID;
#else
            CurrentPlatform = IsGame32Bit() ? MonkiiPlatformAttribute.CompatiblePlatforms.WINDOWS_X86 : MonkiiPlatformAttribute.CompatiblePlatforms.WINDOWS_X64; // Temporarily
#endif
            CurrentDomain = IsGameIl2Cpp() ? MonkiiPlatformDomainAttribute.CompatibleDomains.IL2CPP : MonkiiPlatformDomainAttribute.CompatibleDomains.MONO;
        }

        public static string BaseDirectory { get; private set; }
        public static string GameDirectory { get; private set; }
        public static string GamePackage { get; private set; }
        public static string MonkiiLoaderDirectory { get; private set; }
        public static string UserDataDirectory { get; private set; }
        public static string UserLibsDirectory { get; private set; }
        public static MonkiiPlatformAttribute.CompatiblePlatforms CurrentPlatform { get; private set; }
        public static MonkiiPlatformDomainAttribute.CompatibleDomains CurrentDomain { get; private set; }
        public static MonkiiGameAttribute CurrentGameAttribute { get; private set; }
        ...
        public static void SetCurrentDomainBaseDirectory(string dirpath, AppDomain domain = null)
        {
            ...
            catch (Exception ex) { MonkiiLogger.Warning($"AppDomainSetup.ApplicationBase Exception: {ex}"); }
            Directory.SetCurrentDirectory(dirpath);
        }

        public static MonkiiBase GetMonkiiFromStackTrace()
        {
            StackTrace st = new(3, true);
            return GetMonkiiFromStackTrace(st);
        }

        public static MonkiiBase GetMonkiiFromStackTrace(StackTrace st, bool allFrames = false)
        {
            if (st.FrameCount <= 0)
                return null;
            ...
            if (allFrames)
            {
                ...
                {
                    MonkiiBase ret = CheckForMonkiiInFrame(frame);
                    if (ret != null)
                        return ret;
                }
                return null;
            ...
            MonkiiBase output = CheckForMonkiiInFrame(st);
            if (output == null)
                output = CheckForMonkiiInFrame(st, 1);
            if (output == null)
                output = CheckForMonkiiInFrame(st, 2);
            return output;
        }

        private static MonkiiBase CheckForMonkiiInFrame(StackTrace st, int frame = 0)
        {
            StackFrame sf = st.GetFrame(frame);
            if (sf == null)
                return null;

            return CheckForMonkiiInFrame(sf);
        }

        private static MonkiiBase CheckForMonkiiInFrame(StackFrame sf)
            ...
            => GetMonkiiFromAssembly(((MethodBase)StackFrameGetMethod.Invoke(sf, new object[0]))?.DeclaringType?.Assembly);

        private static MonkiiBase GetMonkiiFromAssembly(Assembly asm)
            => asm == null ? null : MonkiiHandler.Plugins.Cast<MonkiiBase>().FirstOrDefault(x => x.Assembly == asm) ?? MonkiiHandler.Mods.FirstOrDefault(x => x.Assembly == asm);
        ...
        public static T ParseJSONStringtoStruct<T>(string jsonstr)
        {
            ...
            {
                MonkiiLogger.Error($"Exception while Decoding JSON String to JSON Variant: {ex}");
                return default;
            }
            if (jsonarr == null)
                return default;
            ...
            catch (Exception ex) { MonkiiLogger.Error($"Exception while Converting JSON Variant to {typeof(T).Name}: {ex}"); }
            return returnobj;
        }
        ...
        public static T[] PullAttributesFromAssembly<T>(Assembly asm, bool inherit = false) where T : Attribute
        {
            ...
            foreach (Attribute att in att_tbl)
            {
                ...
                if ((attType == requestedType)
                    ...
                    || ((attAssemblyName.Equals("MonkiiLoader")
                        || attAssemblyName.Equals("MonkiiLoader.ModHandler"))
                        && (requestedAssemblyName.Equals("MonkiiLoader")
                        || requestedAssemblyName.Equals("MonkiiLoader.ModHandler"))
...
#else
        [Obsolete("MonkiiLoader.MonkiiUtils.GetUnityVersion() is obsolete. Please use MonkiiLoader.InternalUtils.UnityInformationHandler.EngineVersion instead.")]
        public static string GetUnityVersion() => UnityInformationHandler.EngineVersion.ToStringWithoutType();
        [Obsolete("MonkiiLoader.MonkiiUtils.GameDeveloper is obsolete. Please use MonkiiLoader.InternalUtils.UnityInformationHandler.GameDeveloper instead.")]
        public static string GameDeveloper { get => UnityInformationHandler.GameDeveloper; }
        [Obsolete("MonkiiLoader.MonkiiUtils.GameName is obsolete. Please use MonkiiLoader.InternalUtils.UnityInformationHandler.GameName instead.")]
        public static string GameName { get => UnityInformationHandler.GameName; }
        [Obsolete("MonkiiLoader.MonkiiUtils.GameVersion is obsolete. Please use MonkiiLoader.InternalUtils.UnityInformationHandler.GameVersion instead.")]
