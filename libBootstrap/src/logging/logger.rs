...
lazy_static::lazy_static! {
    static ref MELON_LOADER: Arc<CString> = {
...
macro_rules! log_path {
    () => {
        $crate::melonenv::paths::BASE_DIR.clone().0.join("MelonLoader").join("Latest-Bootstrap.log")
    };
}
...
/// logcat shows the timecode and priority itself, so those aren't logged in the console by the bootstrap
...
macro_rules! log_console {
    ($level:expr, $($arg:tt)*) => {
        ...
        unsafe {
            __android_log_write(prio as _, MELON_LOADER.clone().as_ptr(), format.as_ptr());
        }
    };
}
